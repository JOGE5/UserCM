...
const emit = defineEmits(['close', 'chat-updated']);
...
const loadChat = async () => {
    ...
    try {
        const { data } = await axios.get(route('chats.show', props.chatId), {
            headers: { 'X-Requested-With': 'XMLHttpRequest' }
        });
        chat.value = data.chat;
        isMuted.value = data.is_muted;
        isHidden.value = data.is_hidden;
        messages.value = Array.isArray(data.chat.messages) ? [...data.chat.messages] : [];
        setupWebsockets();
        scrollToBottom();
    } catch (e) {
...
const setupWebsockets = () => {
    if (!window.Echo || !chat.value) return;
    window.Echo.join(`chat.${chat.value.id}`)
        .listen('.MessageSent', (e) => {
            if (!messages.value.some(m => m.id === e.message.id)) {
                messages.value.push(e.message);
                scrollToBottom();
                emit('chat-updated', chat.value.id);
            }
        })
        .listen('.UserTyping', (e) => {
...
const teardownWebsockets = () => {
    if (chat.value?.id && window.Echo) {
        window.Echo.leave(`chat.${chat.value.id}`);
    }
...
const isGeneralChat = computed(() => chat.value?.tipo === 'general');

const otherParticipant = computed(() => {
    if (!chat.value || !chat.value.users) return { name: 'Usuario' };
    return chat.value.users.find(u => u.id !== currentUserId) || chat.value.users[0] || { name: 'Usuario' };
});
...
const sendTypingStart = () => {
    if (isTypingSent.value || !chat.value) return;
    isTypingSent.value = true;
    axios.post(route('chats.typing', chat.value.id), { is_typing: true }).catch(() => {});
};

const sendTypingStop = () => {
    if (!chat.value) return;
    isTypingSent.value = false;
    axios.post(route('chats.typing', chat.value.id), { is_typing: false }).catch(() => {});
};
...
const sendMessage = async () => {
    if ((!newMessage.value.trim() && !pendingFile.value) || processing.value || !chat.value) return;
    ...
    try {
        ...
        const { data } = await axios.post(
            route('chats.messages.store', chat.value.id),
            payload
        );
...
const toggleMute = async () => {
    if (processing.value || !chat.value) return;
    processing.value = true;
    try {
        const { data } = await axios.post(route('chats.mute', chat.value.id));
        isMuted.value = data.is_muted;
        emit('chat-updated', chat.value.id);
    } finally {
        processing.value = false;
...
const toggleReaction = async (msg, emoji) => {
    ...
    try {
        const { data } = await axios.post(route('chats.messages.react', [chat.value.id, msg.id]), { emoji });
...
const copyMessage = (msg) => { navigator.clipboard.writeText(msg.contenido || '').catch(() => {}); };
...
<template>
    ...
    <div v-else-if="chat" class="flex-1 flex flex-col h-full bg-white dark:bg-dark-surface relative z-10 overflow-hidden">
        
        <!-- Header del Chat -->
        <div class="px-6 py-4 bg-white/80 dark:bg-dark-surface/80 backdrop-blur-xl border-b border-light-border dark:border-dark-border flex items-center justify-between shrink-0 relative z-30">
            <div class="flex items-center gap-4">
                <button @click="$emit('close')" class="md:hidden p-2 -ml-2 text-gray-400 hover:text-brand-600 transition-colors">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            ...
                            <span v-if="chat.tipo === 'general'" class="px-1.5 py-0.5 bg-brand-500/10 text-brand-600 dark:text-brand-400 text-[8px] font-black uppercase tracking-widest rounded-full border border-brand-500/20">General</span>
                        </div>
                        <span v-if="typingLabel" class="text-[10px] font-bold text-brand-500 uppercase tracking-widest animate-pulse">{{ typingLabel }}</span>
            ...
            <button
                @click="toggleMute"
                :disabled="processing"
                class="flex items-center gap-2 p-2.5 rounded-full transition-all"
                :class="isMuted ? 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20' : 'bg-gray-100 dark:bg-white/5 text-gray-400 hover:text-amber-500'"
        ...
        <!-- Cuerpo del chat -->
        <div class="flex-1 overflow-y-auto px-4 py-6 custom-scrollbar bg-gray-50/50 dark:bg-black/20 relative z-10 space-y-1">
            <div v-if="activeReactionPicker" class="fixed inset-0 z-10" @click="activeReactionPicker = null"></div>

            <div v-if="messages.length" class="flex justify-center mb-6">
                <span class="px-3 py-1 bg-white dark:bg-white/5 border border-light-border dark:border-dark-border rounded-full text-[9px] font-black text-gray-400 uppercase tracking-widest">Chat Iniciado</span>
            </div>

            <div v-for="(msg, idx) in messages" :key="msg.id" class="relative" :class="idx > 0 && messages[idx-1].sender_id === msg.sender_id ? 'mt-0.5' : 'mt-4'" @mouseenter="onMsgEnter(msg.id)" @mouseleave="onMsgLeave()" @click.self="toggleMsgActions(msg.id)">
                <div class="flex items-end gap-2" :class="msg.sender_id === currentUserId ? 'flex-row-reverse' : 'flex-row'">
                    ...
                    <div class="flex-shrink-0 w-8 self-end mb-1">
                        <template v-if="msg.sender_id !== currentUserId && showAvatar(msg, idx)">
                            <img v-if="msg.sender?.profile_photo_url" :src="msg.sender.profile_photo_url" :alt="msg.sender.name" class="w-8 h-8 rounded-full object-cover ring-2 ring-white dark:ring-dark-surface shadow-sm" />
                            <div v-else class="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-black shadow-sm" :style="{ backgroundColor: senderColor(msg.sender_id) }">
                                {{ msg.sender?.name?.charAt(0)?.toUpperCase() }}
                            </div>
                        </template>
                    </div>

                    <div class="relative flex flex-col gap-1" :class="msg.sender_id === currentUserId ? 'items-end max-w-[75%]' : 'items-start max-w-[75%]'">
                        <div v-show="hoveredMsg === msg.id || activeReactionPicker === msg.id" class="absolute bottom-full mb-1 z-20 flex items-center gap-0.5 bg-white dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl shadow-xl px-1.5 py-1" :class="msg.sender_id === currentUserId ? 'right-0' : 'left-0'" @mouseenter="onActionEnter()" @mouseleave="onMsgLeave()">
                            <div class="relative">
                                <button @click.stop="activeReactionPicker = activeReactionPicker === msg.id ? null : msg.id" class="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 hover:text-amber-500 transition-colors">
                                    <SmilePlus class="w-3.5 h-3.5" />
                                </button>
                                <div v-if="activeReactionPicker === msg.id" class="absolute bottom-full mb-1 flex items-center gap-1 p-2 bg-white dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl shadow-2xl z-30" :class="msg.sender_id === currentUserId ? 'right-0' : 'left-0'" @click.stop @mouseenter="onActionEnter()" @mouseleave="onMsgLeave()">
                                    <button v-for="emoji in REACTION_EMOJIS" :key="emoji" @click="toggleReaction(msg, emoji)" class="text-lg leading-none hover:scale-125 transition-transform px-0.5">{{ emoji }}</button>
                                </div>
                            </div>
                            <button @click="startReply(msg)" class="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 hover:text-brand-500 transition-colors">
                                <CornerUpLeft class="w-3.5 h-3.5" />
                            </button>
                            <button v-if="msg.contenido" @click="copyMessage(msg)" class="p-1.5 rounded-xl hover:bg-gray-100 dark:hover:bg-white/10 text-gray-400 hover:text-brand-500 transition-colors">
                                <Copy class="w-3.5 h-3.5" />
                            </button>
                        </div>

                        <span v-if="showSenderName(msg, idx)" class="text-[10px] font-black uppercase tracking-widest px-1" :style="{ color: senderColor(msg.sender_id) }">{{ msg.sender?.name ?? 'Usuario' }}</span>

                        <div v-if="msg.reply_to" class="flex items-start gap-2 px-3 py-2 rounded-2xl max-w-full cursor-pointer opacity-80 hover:opacity-100 transition-opacity" :class="msg.sender_id === currentUserId ? 'bg-brand-500/20 border-l-2 border-brand-200' : 'bg-black/5 dark:bg-white/5 border-l-2 border-brand-400'">
                        ...
                        <template v-if="msg.type === 'product_card' && msg.metadata">
                            <div :class="['overflow-hidden rounded-[1.8rem] shadow-md border w-72', msg.sender_id === currentUserId ? 'rounded-tr-none border-brand-300/40 bg-brand-600' : 'rounded-tl-none border-light-border dark:border-dark-border bg-white dark:bg-black/40']">
                        ...
                        <template v-else>
                            <div :class="['px-5 py-3 rounded-[1.5rem] shadow-sm text-sm leading-relaxed', msg.sender_id === currentUserId ? 'bg-brand-600 text-white rounded-tr-sm' : 'bg-white dark:bg-black/40 text-gray-800 dark:text-gray-200 border border-light-border dark:border-dark-border rounded-tl-sm']">
                                ...
                                <a v-else-if="msg.attachment_path" :href="attachmentUrl(msg.attachment_path)" target="_blank" :download="msg.attachment_name" :class="['flex items-center gap-2 mt-2 px-3 py-2 rounded-xl text-xs font-bold transition-colors', msg.sender_id === currentUserId ? 'bg-white/20 hover:bg-white/30 text-white' : 'bg-gray-100 dark:bg-white/10 hover:bg-gray-200 dark:hover:bg-white/20 text-gray-700 dark:text-gray-200']">
                                    <FileText class="w-4 h-4 shrink-0" />
                                    <span class="truncate max-w-[160px]">{{ msg.attachment_name }}</span>
                                    <Download class="w-3 h-3 shrink-0 ml-auto" />
                                </a>
                            </div>
                            <div class="flex items-center gap-1 text-[8px] font-bold opacity-40 px-1 mt-0.5">
                                <Clock class="w-2.5 h-2.5" />
                                {{ formatMsgTime(msg.created_at) }}
                            </div>
                        </template>
                        ...
                        <div v-if="groupReactions(msg.reactions).length" class="flex flex-wrap gap-1 mt-0.5">
                            <button v-for="r in groupReactions(msg.reactions)" :key="r.emoji" @click="toggleReaction(msg, r.emoji)" class="flex items-center gap-1 px-2.5 py-1 rounded-full border text-xs font-bold transition-all hover:scale-105 active:scale-95" :class="r.reacted ? 'bg-brand-500/10 border-brand-500/30 text-brand-600 dark:text-brand-400 shadow-sm' : 'bg-white dark:bg-black/40 border-light-border dark:border-dark-border text-gray-600 dark:text-gray-300 hover:border-brand-500/30'" :title="r.reacted ? 'Quitar reacción' : 'Reaccionar'">
        ...
        <!-- Input Area -->
        <div class="p-4 bg-white dark:bg-dark-surface border-t border-light-border dark:border-dark-border shrink-0">
            ...
            <div v-if="showEmojiPicker" class="absolute bottom-[80px] left-4 mb-2 z-50 p-3 bg-white dark:bg-dark-surface border border-light-border dark:border-dark-border rounded-2xl shadow-2xl grid grid-cols-10 gap-1 w-80">
                <button v-for="emoji in EMOJIS" :key="emoji" @click="insertEmoji(emoji)" class="text-xl hover:scale-125 transition-transform p-0.5 leading-none">{{ emoji }}</button>
            </div>
            ...
            <div class="relative group flex items-end gap-2">
                ...
                ></textarea>
                <button @click="sendMessage" :disabled="(!newMessage.trim() && !pendingFile) || processing" class="flex-shrink-0 p-3.5 bg-brand-600 text-white rounded-full shadow-lg shadow-brand-500/30 hover:bg-brand-500 disabled:opacity-30 disabled:grayscale transition-all hover:scale-105 active:scale-95">
                    <Send class="w-5 h-5" />
                </button>
            </div>
            ...
            <div class="flex items-center gap-3 mt-3 px-2">
                <button @click="triggerImage" class="flex items-center gap-1.5 text-[9px] font-black text-gray-400 hover:text-brand-500 uppercase tracking-widest transition-colors">
                    <Image class="w-3.5 h-3.5" /> Imagen
                </button>
                <button @click="triggerFile" class="flex items-center gap-1.5 text-[9px] font-black text-gray-400 hover:text-brand-500 uppercase tracking-widest transition-colors">
                    <Paperclip class="w-3.5 h-3.5" /> Archivo
                </button>
                <button @click="showEmojiPicker = !showEmojiPicker" :class="['flex items-center gap-1.5 text-[9px] font-black uppercase tracking-widest transition-colors', showEmojiPicker ? 'text-brand-500' : 'text-gray-400 hover:text-brand-500']">
                    <Smile class="w-3.5 h-3.5" /> Emoji
                </button>
                <div class="flex-1"></div>
                <span class="text-[9px] font-bold text-gray-300 dark:text-gray-600 italic hidden sm:block">Enter para enviar</span>
            </div>
        </div>
    </div>
</template>
