    ...
    <div class="flex flex-col items-center min-h-screen pt-6 bg-gray-100 sm:justify-center sm:pt-0">
        <div class="w-full px-6 py-4 mt-6 overflow-hidden bg-white shadow-md custom-auth-form sm:max-w-md sm:rounded-lg">
            <form @submit.prevent="submit" class="form">
                ...
                <div class="inputForm">
                    <svg height="20" viewBox="0 0 24 24" width="20" xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <div v-if="$page.props.jetstream.hasTermsAndPrivacyPolicyFeature" class="flex-row mt-4">
                    <div>
                        <input type="checkbox" id="terms" v-model="form.terms" required />
                        <label for="terms" class="text-sm text-gray-600 ms-2">
                            I agree to the <a target="_blank" :href="route('terms.show')" class="span">Terminos y condiciones</a> and <a target="_blank" :href="route('policy.show')" class="span">Privacy Policy</a>
                        </label>
                    </div>
                </div>
                <InputError v-if="$page.props.jetstream.hasTermsAndPrivacyPolicyFeature" class="mt-2 text-sm text-red-600" :message="form.errors.terms" />

                <button class="button-submit" :disabled="form.processing">
                    Ingresar
                </button>

                <p class="p">Ya tienes una cuenta? <Link :href="route('login')" class="span">Inciar Sesión</Link></p>
